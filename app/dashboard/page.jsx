    ...
    >
      ...
      <aside className={`hidden md:flex md:w-64 border-r flex-col py-6 ${theme === 'dark' ? 'bg-black border-gray-800' : 'bg-white border-gray-200'}`}>
        ...
        <nav className="px-2 text-sm space-y-1">
          <a className={`block px-4 py-2 rounded-lg ${theme === 'dark' ? 'hover:bg-gray-800' : 'hover:bg-gray-100'}`}>Overview</a>
          <a className={`block px-4 py-2 rounded-lg ${theme === 'dark' ? 'hover:bg-gray-800' : 'hover:bg-gray-100'}`}>Profile</a>
          <a className={`block px-4 py-2 rounded-lg ${theme === 'dark' ? 'hover:bg-gray-800' : 'hover:bg-gray-100'}`}>Products</a>
          <a className={`block px-4 py-2 rounded-lg ${theme === 'dark' ? 'hover:bg-gray-800' : 'hover:bg-gray-100'}`}>Reviews</a>
          {/* Settings nav item */}
          <button
            type="button"
              ...
              <form onSubmit={handleEditSubmit} className={`mt-6 p-4 rounded-xl space-y-3 border ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
                <div>
                  ...
                  <input
                    ...
                    className={
                      `w-full rounded-lg px-3 py-2 outline-none focus:border-blue-400 ${theme === 'dark' ? 'bg-gray-900 border border-gray-600 text-white' : 'bg-white border border-gray-300 text-gray-900'} ` +
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleEditChange}
                    className={
                      `w-full rounded-lg px-3 py-2 outline-none resize-y focus:border-blue-400 ${theme === 'dark' ? 'bg-gray-900 border border-gray-600 text-white' : 'bg-white border border-gray-300 text-gray-900'} ` +
          ...
          <div className="mb-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {createdProducts.map((prod) => (
                <div
                  key={prod.id}
                  className={`rounded-xl border transition shadow-sm flex flex-col gap-2 p-4 ${theme === 'dark' ? 'bg-gray-800 border-gray-700 hover:bg-gray-700' : 'border-gray-200 bg-gray-50 hover:bg-gray-100'}`}
                >
                  {prod.imageUrl && (
                    <img
                      src={prod.imageUrl}
                  ...
                  {prod.description && (
                    <div className={theme === 'dark' ? 'text-gray-300 text-sm' : 'text-gray-700 text-sm'}>{prod.description}</div>
                  )}
                  {typeof prod.price !== "undefined" && (
                    <div className="font-semibold text-green-400 dark:text-green-400 text-green-600">
                      Price: ₹{prod.price}
                    </div>
                  )}
                  {prod.createdAt && (
